...
class MathContext:
    """Retain classification of math mode and nested non-math or upright regions."""
...
class ContextTracker:
    """Track math, non-math, unit, and upright nesting depths across tokens."""
    ...
    def handle_token(self, token: Token) -> None:
        ...
        elif token.kind != "comment" and not (token.kind == "text" and token.value.isspace()):
            self.non_math.reset_pending()
            self.unit.reset_pending()
            self.upright.reset_pending()
