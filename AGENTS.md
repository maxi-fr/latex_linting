...
uv run ty check src/latex_linting/foo.py
...
Ruff runs `select = ["ALL"]`, so ill-fitting rules get suppressed, though fixing the code beats silencing the check. One rule code, one line, with a reason: `# noqa: ARG002 -- reason`.
